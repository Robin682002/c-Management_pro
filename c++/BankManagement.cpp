...
void temp :: depositAmount(){
	...
	while(!file.eof()) {
		if(accNo==search){  //comp acc with search
			cout<<"\ncurrent amount :: "<<amount; //inc amount
			amount=amount + newAmount;
			cout<<"\nupdated amount :: "<<amount<<endl; //uodated amount
			file1<<accNo<<"\t"<<name<<"\t"<<Fname<<"\t"<<cnic<<"\t"<<P_no<<"\t"<<email<<"\t"<<amount<<endl; //if accno match file 1 is updated to main file
		}else{
